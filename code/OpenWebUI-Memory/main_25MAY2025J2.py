...
async def chat_completion(
    request: Request,
    form_data: ChatCompletionForm = Depends(ChatCompletionForm.as_form),  # Use as_form for multipart
    ...
    async def as_form(
        ...
        model_item: Optional[Dict[str, Any]] = Form({}),
        chat_id: Optional[str] = Form(None),
        id: Optional[str] = Form(None),
        session_id: Optional[str] = Form(None),
        messages: List[Dict[str, Any]] = Form(...),  # Required
        tool_ids: Optional[List[str]] = Form(None),
        tool_servers: Optional[List[str]] = Form(None),
        files: Optional[List[str]] = Form(None),
        features: Optional[Dict[str, Any]] = Form(None),
        variables: Optional[Dict[str, Any]] = Form(None),
        params: Optional[Dict[str, Any]] = Form(None),
        background_tasks: Optional[Any] = Form(None)
    ) -> "ChatCompletionForm":
        return cls(
            model=model,
    ...
    try:
        model_item = form_data.model_item or {}  # Changed from pop
        tasks = form_data.background_tasks  # Changed from pop
        ...
        form_data_dict = form_data.dict(exclude_unset=True)  # Convert to dict for compatibility
