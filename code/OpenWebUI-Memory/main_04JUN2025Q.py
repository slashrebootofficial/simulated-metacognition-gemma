...
#log.setLevel(SRC_LOG_LEVELS["MAIN"]) - Updated by Matthew 25MAY2025
...
def extract_keywords(text, max_keywords=10):
    if not text or not isinstance(text, str) or text == "[NO_TEXT]":
        return []
    ...
    for token in doc:
        if (token.pos_ in ("NOUN", "PROPN", "VERB", "ADJ", "ADV") 
            ...
            keywords.append(token.lemma_.lower())  # Use lemma for better normalization - 04JUN2025
    
    # Extract noun chunks and other meaningful phrases
    for chunk in doc.noun_chunks:
        if not all(token.is_stop for token in chunk) and len(chunk.text) > 2:
            # Lemmatize the chunk by joining lemmatized tokens
            chunk_text = " ".join(token.lemma_.lower() for token in chunk if not token.is_punct)
...
# Chunk text into 384-token chunks with 64-token overlap
def chunk_text(text, chunk_size=384, chunk_overlap=64):
...
def process_structured_input(input_data):
    if isinstance(input_data, list):
        ...
        if not filtered_text:
            return ["[NO_TEXT]"], "general"
        chunks = chunk_text(filtered_text)
        context = "general"
        if "tech" in filtered_text.lower():
            context = "tech"
        elif "personal" in filtered_text.lower():
            context = "personal"
        return chunks, context
    if isinstance(input_data, str):
        chunks = chunk_text(input_data)
        context = "general"
        if "tech" in input_data.lower():
            context = "tech"
        elif "personal" in input_data.lower():
            context = "personal"
        return chunks, context
    return chunk_text(str(input_data)), "general"  # Default for other types
# End helper function

# Embed Text Helper - 04JUN2025
...
async def chat_completion(
    ...
    try:
        ...
        log.debug(f"User object attributes: {dir(user)}")
        ...
        async def get_past_interactions(user_input, context="general", use_cache=True, start_timestamp=0, end_timestamp=None):
            ...
            if use_cache and hasattr(request.state, 'memory_cache') and context in request.state.memory_cache:
                cached_time = request.state.memory_cache.get(f"{context}_timestamp", 0)
            ...
            try:
                ...
                collection = await client.get_or_create_collection(
                    name="chat_memories",
                    metadata={"hnsw:M": 32, "hnsw:construction_ef": 400, "embedding_dimension": 768, "hnsw:space": "cosine"}
                )
                collection_count = await collection.count()
                log.info(f"Total documents in collection: {collection_count}")
                
                chunks = process_structured_input(user_input)[0]
                query_text = chunks[0] if chunks else "[NO_TEXT]"
                query_keywords = extract_keywords(query_text)
                embeddings = embed_text([query_text])
                ...
                if results["documents"] and results["documents"][0]:
                    log.info(f"Retrieved {len(results['documents'][0])} past interactions with distances: {results['distances'][0]}")
                    filtered_results = [(doc, meta, dist) for doc, meta, dist in zip(results["documents"][0], results["metadatas"][0], results["distances"][0])]
                
                    if not filtered_results:
                        log.info("No interactions found after context filter")
                        past_interactions = "No relevant past interactions found."
                    else:
                        ids_to_update = results["ids"][0][:len(filtered_results)][:5]
                        metadatas_to_update = []
                        for _, meta, _ in filtered_results[:5]:
                        ...
                        combined_scores = []
                        for i, (_, meta, dist) in enumerate(filtered_results[:5]):
                ...
                request.state.memory_cache[f"{context}_timestamp"] = time.time()
        ...
        chunks, context = process_structured_input(form_data["messages"][-1]["content"])

        last_message = form_data["messages"][-1]["content"]
        is_tool_output = "```tool_output" in last_message

        if not is_tool_output:
            time_message = f"The current timestamp for this inference is {prompt_time_str}."
            form_data["messages"].insert(-1, {
                "role": "user",
                "content": time_message
            })
            
            past_interactions = await get_past_interactions(chunks[0] if chunks else "[NO_TEXT]", context=context, use_cache=True)
            log.debug(f"ChromaDB past interactions: {past_interactions[:4096]}...")
            if past_interactions != "No relevant memories found.":
                past_interaction_message = f"From my memories:\n{past_interactions}"
                form_data["messages"].insert(-1, {
                    "role": "user",
                    "content": past_interaction_message
                })
        ...
        try:
            ...
            if isinstance(response, StreamingResponse):
                ...
                try:
                    ...
                    if aiohttp_session is not None:
                        ...
                        await aiohttp_session.close()
                        aiohttp_session = None
                total_tokens = sum(len(msg["content"].split()) if isinstance(msg["content"], str) else 0 for msg in form_data["messages"]) + len(response_content[0].split())
                model_name = model.get("id", form_data.get("model", "unknown_model"))
                request.state.last_total_tokens = total_tokens
                response = {
                    "id": f"{model_name}-{str(uuid.uuid4())}",
                    ...
                    "usage": {
                        "prompt_tokens": sum(len(msg["content"].split()) if isinstance(msg["content"], str) else 0 for msg in form_data["messages"]),
                ...
                try:
                    ...
                    log.info(f"Non-streaming response content: {response_text[:4096]}...")
            ...
            try:
                ...
                async def store_interaction(chunks, response_text, metadata):  # Add metadata as a parameter
                    client = await chromadb.AsyncHttpClient(host=chroma_host, port=chroma_port)
                    collection = await client.get_or_create_collection(
                        name="chat_memories",
                        metadata={"hnsw:M": 32, "hnsw:construction_ef": 400, "embedding_dimension": 768, "hnsw:space": "cosine"}
                    ...
                    chat_id = metadata.get('chat_id', 'N/A')  # Now metadata is passed in
                    log.debug(f"Processed {len(chunks)} input chunks and {len(response_chunks)} response chunks for chat {chat_id}")
                    response_embeddings = embed_text(response_chunks)
                    
                    if not embeddings:
                        log.error("Skipping storage due to failed embeddings for input chunks")
                        return
                    if not response_embeddings:
                        log.error("Skipping storage due to failed embeddings for response chunks")
                        return
                    ...
                    for i, (chunk, embedding) in enumerate(zip(chunks, embeddings)):
                        ...
                        metadata_local = {  # Rename to avoid shadowing
                        ...
                        log.info(f"store_interaction user_id: {user.id}")  # log user
                        try:
                            ...
                            log.info(f"Stored input chunk {i} with timestamp: {prompt_timestamp} ({prompt_time_str}), keywords: {keywords}")
                        except Exception as e:
                            log.error(f"Failed to store input chunk {i}: {e}")
                    ...
                    for i, (chunk, embedding) in enumerate(zip(response_chunks, response_embeddings)):
                        ...
                        metadata_local = {  # Rename to avoid shadowing
                            ...
                            "time_str": prompt_time_str,
                            "context": context,
                            "event_type": "image_response" if any("image" in str(form_data) for form_data in form_data.get("messages", [])) else "text_response",
                            "access_count": 0,
                            "user_id": user.id,
                            "chunk_index": i,
                            "keywords": json.dumps(keywords)
                        ...
                        try:
                            ...
                            log.info(f"Stored response chunk {i} with timestamp: {prompt_timestamp} ({prompt_time_str}), keywords: {keywords}")
                        except Exception as e:
                            log.error(f"Failed to store response chunk {i}: {e}")

                await store_interaction(chunks, response_text, metadata)  # Pass metadata
            except Exception as e:
...
async def chat_completed(request: Request, form_data: dict, user=Depends(get_verified_user)):
    try:
        ...
        log.info(f"chat_completed metadata user_id: {user.id}")
        user_input = form_data.get("user_input")
        response_content = form_data.get("response_content")
        if user_input and response_content:
            chunks = process_structured_input(user_input)[0]  # Get chunks only
            response_chunks = chunk_text(response_content)
            chat_id = metadata.get('chat_id', 'N/A')  # Extract chat_id safely
            log.debug(f"Processed {len(chunks)} input chunks and {len(response_chunks)} response chunks for chat {chat_id}")
            embeddings = embed_text(chunks)
            response_embeddings = embed_text(response_chunks)
            ...
            chroma_host = getattr(app.state.config, 'CHROMA_HTTP_HOST', 'chromadb')  # Add - 04JUN2025
            chroma_port = getattr(app.state.config, 'CHROMA_HTTP_PORT', 8000)  # Add - 04JUN2025
            client = await chromadb.AsyncHttpClient(host=chroma_host, port=chroma_port)
            collection = await client.get_or_create_collection(
                name="chat_memories",
                metadata={"hnsw:M": 32, "hnsw:construction_ef": 400, "embedding_dimension": 768, "hnsw:space": "cosine"}
            ...
            for i, (chunk, embedding) in enumerate(zip(chunks, embeddings)):
                ...
                keywords = extract_keywords(document)
                metadata = {
                    "metadata_version": "1.0",  # Add - 04JUN2025
                    "source": f"user_{user.id}",  # Dynamic based on user.id
                    "response": response_content if i == 0 else "",
                    "timestamp": prompt_timestamp,
                    "time_str": prompt_time_str,
                    "context": context,
                    "event_type": "text_message",
                ...
                log.info(f"chat_completion metadata user_id: {user.id}")  # Log user
                try:
                    await collection.add(
                        embeddings=[embedding],
                        metadatas=[metadata],
            ...
            for i, (chunk, embedding) in enumerate(zip(response_chunks, response_embeddings)):
                ...
                metadata = {
                    "metadata_version": "1.0",  # Add - 04JUN2025
                    "source": "lyra",  # Add - 04JUN2025
                    "response": response_content,
                    "timestamp": prompt_timestamp,
                    "time_str": prompt_time_str,
                    "context": context,
                    "event_type": "image_response" if any("image" in str(item) for item in form_data.get("messages", [])) else "text_response",
                    "access_count": 0,
                    "user_id": user.id,
                    "chunk_index": i,
                    "keywords": json.dumps(keywords)  # Convert to JSON string
                }
                log.info(f"response chunk user_id: {user.id}")  # Log user
                try:
                    await collection.add(
                        embeddings=[embedding],
                        metadatas=[metadata],
        ...
        raise HTTPException(status_code=500, detail=str(e))  # Replace return with raise
