...
async def chat_completion(
    ...
    try:
            ...
            try:
                ...
                if results["documents"]:
                    ...
                    for i, _ in combined_scores:
                        ...
                        raw_image_base64 = meta.get('raw_image_base64', 'N/A')
                        # Calculate and log retrieved bytes for base64 string
                        if raw_image_base64 and raw_image_base64 != "N/A":
                            try:
                                byte_size = len(raw_image_base64.encode('utf-8'))
                                log.info(f"Retrieved raw_image_base64 with {byte_size} bytes for memory at {time_str}")
                            except Exception as e:
                                log.error(f"Failed to calculate retrieved bytes: {e}")
                        else:
                            log.info(f"No base64 image retrieved for memory at {time_str}")
                        # Keep truncation for logging consistency
                        display_base64 = raw_image_base64[:50] + "..." if raw_image_base64 and raw_image_base64 != "N/A" else "N/A"
                        access_count = int(meta.get('access_count', 0))
                        past_interactions += f"Past user [{time_str}]: {doc}\nLyra: {meta['response']}\nRaw Image Base64: {display_base64}\nAccess Count: {access_count}\n\n"
                else:
        ...
        # Check for image-related past interactions and inject raw_image_base64 if present
        if past_interactions and past_interactions != "No relevant past interactions found.":
            # Parse past interactions to find raw_image_base64
            raw_image_base64 = None
            for line in past_interactions.split("\n"):
                if "Raw Image Base64:" in line and "N/A" not in line:
                    raw_image_base64 = line.split("Raw Image Base64: ")[1].split("...")[0]
                    break
            if raw_image_base64:
                # Inject the image back into the messages as an image_url
                form_data["messages"].insert(-1, {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": "Here’s the image you previously saw:"},
                        {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{raw_image_base64}"}}
                    ]
                })
            form_data["messages"].insert(-1, {
                "role": "system",
                ...
                try:
                    ...
                    async def store_interaction(user_input, response_content, base64_string):
                        ...
                        if base64_string:
                            ...
                            try:
                                byte_size = len(base64_string.encode('utf-8'))
                                log.info(f"Storing raw_image_base64 with {byte_size} bytes")
                            except Exception as e:
                                log.error(f"Failed to calculate stored bytes: {e}")
                        try:
