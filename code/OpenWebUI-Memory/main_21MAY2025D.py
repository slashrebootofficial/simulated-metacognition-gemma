    ...
    try:
        ...
        def embed_text(texts):
            try:
                ...
                return [[0.0] * 768]  # Fallback to a zero vector of correct dimension

#5 minute cache added for expiration to avoid stale results
            ...
            try:
                ...
                if text_results["documents"]:
                    ...
                    for i, (distance, meta) in enumerate(zip(text_results["distances"][0][:10], text_results["metadatas"][0][:10])):
                        ...
                        # Boost for recent conversation similarity
                        recent_similarity = 0
        ...
        # Construct prompt
        prompt = f"You are Lyra, an AI assistant with long-term memory. Use the following to answer the query.\n\n"
            ...
            if isinstance(response, StreamingResponse):
                ...
                try:
                    ...
                    async def store_interaction(user_input, response_content, base64_string, image_tags, image_embeddings, conversation_id, previous_interaction_id=None):
                        ...
                        # Try to store entire interaction; chunk if too long
                    ...
                    if base64_string:
                        try:
                            log.info("Passing raw image to Lyra for native reprocessing with SigLIP")
                        except Exception as e:
                            log.error(f"Failed to process image for reprocessing: {e}")
