...
def init_app(app):
    app.state.embedding_model = SentenceTransformer('all-MiniLM-L6-v2', device='cuda:2')  # Third GPU
    app.state.chroma_client = HttpClient(host="chromadb", port=8000)
    app.state.thinking_loop_active = False
    app.state.loop_state_file = "/app/backend/data/lyra_loop_state.pkl"  # Docker volume
    app.state.embedding_cache = TTLCache(maxsize=1000, ttl=3600)  # 1-hour cache
...
async def calculate_confidence(app, response, source, input_text):
    ...
    log.debug(f"Confidence for {source}: {confidence} (similarity={similarity}, emotional_score={emotional_score})")
...
async def embed_and_query(app, user_input, collection_name="chat_memories", n_results=5, min_confidence=0.5):
    ...
    if cache_key in app.state.embedding_cache:
        log.debug(f"Cache hit for query: {cache_key}")
    ...
    try:
        ...
        log.debug(f"Stored query result in cache: {cache_key}")
...
async def embed_and_store(app, user_input, response_content, source="user_chat", collection_name="chat_memories"):
    try:
        ...
        for i, (chunk, embedding) in enumerate(zip(chunks, embeddings)):
            chunk_response = response_content if i == 0 else ""  # Store response with first chunk
...
async def thinking_loop(app):
    os.environ["CUDA_VISIBLE_DEVICES"] = "0,1"  # Pin lyra_4 to first two GPUs
    ...
    try:
        ...
        system_prompt = await embed_and_query(app, "system_prompt", collection_name="lyra_identity", n_results=len(wrap(LYRA_SYSTEM_PROMPT, 500)))
        system_prompt = "\n".join([meta["response"] for meta in system_prompt["metadatas"][0]]) if system_prompt else LYRA_SYSTEM_PROMPT
        while True:
            if time.time() - state["last_run"] < 600:  # 10-minute cycle
