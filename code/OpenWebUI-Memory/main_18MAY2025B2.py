...
# Process structured input, return raw text
...
async def chat_completion(
    ...
    try:
        ...
        async def get_past_interactions(user_input, context="general", use_cache=True):
            ...
            try:
                ...
                collection = await client.get_or_create_collection(
                    name="chat_memories",
                    metadata={"hnsw:M": 32, "hnsw:ef_construction": 400, "dimension": 768}
                )
        ...
        filtered_user_input, _, _ = process_structured_input(form_data["messages"][-1]["content"])
        context = "general"  # Simplified context since no base64 image handling
        ...
        try:
            ...
            if isinstance(response, StreamingResponse):
                ...
                try:
                    ...
                    log.info(f"Non-streaming response content: {response_content[:4096]}...")
                ...
                try:
                    ...
                    async def store_interaction(user_input, response_content):
                        filtered_input, _, _ = process_structured_input(user_input)
                        document = filtered_input.strip() or "[NO_TEXT]"
                        client = await chromadb.AsyncHttpClient(host=chroma_host, port=chroma_port)
                        collection = await client.get_or_create_collection(
                            name="chat_memories",
                            metadata={"hnsw:M": 32, "hnsw:ef_construction": 400, "dimension": 768}
                    ...
                    await store_interaction(filtered_user_input, response_content)
