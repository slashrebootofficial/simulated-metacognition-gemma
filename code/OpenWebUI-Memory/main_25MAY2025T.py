    ...
    try:
        ...
        async def get_past_interactions(user_input, context="general", use_cache=True):
            ...
            try:
                ...
                # Query by keywords (JSON strings)
                results = await collection.query(
                    query_embeddings=[query_embedding],
                    n_results=5,
                    where={"keywords": {"$in": [json.dumps([kw]) for kw in query_keywords]}} if query_keywords else {},
                ...
                if results["documents"] and results["documents"][0]:
                    ...
                    if not filtered_results:
                        ...
                        ids_to_update = [str(uuid.uuid4()) for _ in filtered_results[:5]]
                        metadatas_to_update = []
                        ...
                        for i, _ in combined_scores:
                            ...
                            access_count = int(meta.get('access_count', 0))
                            keywords = json.loads(meta.get('keywords', '[]'))  # Parse JSON string
