...
# Strip base64 content from text
def strip_base64_content(text: str) -> str:
    if not isinstance(text, str):
        return text if text else "[NO_TEXT]"
    base64_pattern = r'data:[^;]+;base64,[^)]+'
    cleaned_text = re.sub(base64_pattern, '[BASE64_CONTENT_REMOVED]', text)
    return cleaned_text.strip() or "[NO_TEXT]"

# Retrieve conversational memory
async def retrieve_memory(query: str, user_id: str, n_results: int = 3) -> str:
    try:
        if not query.strip():
            return json.dumps({"error": "Query cannot be empty"})
        if not user_id:
            return json.dumps({"error": "User ID cannot be empty"})
        ...
        if not query_embedding:
            log.error("Failed to embed query for memory retrieval")
        ...
        conversation_ids = set(meta["conversation_id"] for meta in results["metadatas"][0])
        all_chunks = await collection.query(
            query_embeddings=None,
            where={"conversation_id": {"$in": list(conversation_ids)}},
            include=["documents", "metadatas"]
        )
        ...
        conversations = {}
        for doc, meta in zip(all_chunks["documents"][0], all_chunks["metadatas"][0]):
        ...
        for conv_id, chunks in conversations.items():
            chunks.sort(key=lambda x: x["chunk_index"])
            expected_indices = set(range(len(chunks)))
            actual_indices = set(chunk["chunk_index"] for chunk in chunks)
            if expected_indices != actual_indices:
                log.warning(f"Missing chunks for conversation_id {conv_id}: {expected_indices - actual_indices}")
            
            conversation_text = " ".join(chunk["content"] for chunk in chunks)
...
def parse_tool_calls(response_text: str) -> List[Dict[str, Any]]:
    ...
    for match in matches:
        try:
            ...
            if func_match and func_match.group(1) in TOOL_FUNCTIONS:
                ...
                for arg in args_str.split(","):
                    if "=" in arg:
                        ...
                        args[key] = value
                if func_name == "Retrieve_Memory":
                    if not args.get("query") or not args.get("user_id"):
                        log.error(f"Missing required args for Retrieve_Memory: {args}")
                        continue
                tool_calls.append({"tool": func_name, "args": args})
        except Exception as e:
            log.error(f"Failed to parse tool call: {match}, error: {e}")
    return tool_calls
...
# Store conversational memory
async def store_conversation_memory(user_input: str, response_text: str, metadata: Dict[str, Any]):
    clean_user_input = strip_base64_content(user_input)
    clean_response_text = strip_base64_content(response_text)
    
    chunks = chunk_text(clean_user_input)
    response_chunks = chunk_text(clean_response_text)
    embeddings = embed_text(chunks)
    response_embeddings = embed_text(response_chunks)
...
async def chat_completion(
    ...
    try:
        ...
        form_data["messages"].append({"role": "system", "content": f"Current user ID: {metadata['user_id']}"})
        form_data["messages"].append({"role": "system", "content": f"Current time: {prompt_time_str}"})
        ...
        while iteration < max_tool_iterations:
            ...
            if isinstance(response, StreamingResponse):
                response_content = [""]
                async for chunk in stream_generator(response, response_content, None):  # aiohttp_session passed separately
                    yield chunk
            ...
            for tool_call in tool_calls:
                ...
                args = tool_call["args"]
                log.info(f"Executing tool: {tool_name} with args: {args}")
        ...
        # Process final response and yield as a streaming chunk
        ...
        if "aiohttp_session" in locals():
            await aiohttp_session.close()
        error_response = {
            "error": {"message": str(e), "type": "HTTPException", "status_code": status.HTTP_400_BAD_REQUEST}
        }
