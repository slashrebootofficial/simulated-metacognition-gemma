...
from open_webui.socket.main import app as socket_app, periodic_usage_pool_cleanup
from open_webui.routers import audio, images, ollama, openai, retrieval, pipelines, tasks, auths, channels, chats, folders, configs, groups, files, functions, memories, models, knowledge, prompts, evaluations, tools, users, utils
from open_webui.routers.retrieval import get_embedding_function, get_ef, get_rf
...
conversation_state = {}  # {conversation_id: last_interaction_id}
...
try:
    chroma_client = chromadb.AsyncHttpClient(
        ...
        settings=ChromaSettings(chroma_api_path="/api/v2")
    )
        ...
        for item in input_data:
            if isinstance(item, dict):
                if item.get('type') == 'text' and 'text' in item:
                    ...
                    # Skip base64 images, expect raw images via UploadFile
...
def split_text_by_tokens(text, chunk_size=384, chunk_overlap=64):
    if not tokenizer:
        log.error("Tokenizer unavailable, falling back to text")
        return [text]
    try:
...
#        log.info("Connecting to ChromaDB at %s:%s", chroma_host, chroma_port)
            ...
            try:
                ...
                for text in texts:
                    ...
                    if not chunks:
                        chunks = [text]
                    # Embed each chunk
                    chunk_embeddings = embedding_model.encode(chunks, convert_to_tensor=False).tolist()
        ...
        def extract_entities(text):
            ...
            try:
                ...
                return [{"text": ent[0], "label": ent[1]} for ent in entities]
            except Exception as e:
                log.error(f"Entity extraction error: {e}")
                return []
        def generate_caption(image):
            try:
                inputs = blip_processor(images=image, return_tensors="pt").to('cuda' if blip_model.device.type == 'cuda' else 'cpu')
                with torch.no_grad():
        ...
        def embed_image(image):
            try:
                inputs = clip_processor(images=image, return_tensors="pt").to('cuda' if clip_model.device.type == 'cuda' else 'cpu')
                with torch.no_grad():
                    embedding = clip_model.get_image_features(**inputs).cpu().numpy().tolist()[0]
                return embedding
            except Exception as e:
    ...
    try:
        ...
        if results["documents"]:
            ...
            for interaction_id, data in interactions.items():
                ...
                # Recent interaction similarity (last 3 interactions)
                recent_score = 0
        ...
        filtered_user_input, uploaded_image = process_structured_input(form_data["messages"][-1]["content"])
        ...
        past_interactions = await get_past_interactions(filtered_user_input, context=context, conversation_id=conversation_id)
        ...
        if image_file:
            ...
            past_image_interactions = await get_past_image_interactions(image=image, context="image_related", conversation_id=conversation_id)
            log.info(f"ChromaDB past image interactions: {past_image_interactions[:4096]}...")
            form_data["messages"].insert(-1, {
                "role": "system",
                "content": f"Image caption: {caption}\nEntities: {', '.join([e['text'] for e in caption_entities])}\nPast image interactions:\n{past_image_interactions}"
            })
        ...
        try:
            ...
            if isinstance(response, StreamingResponse):
                response_content = [""]
                try:
                    await stream_generator(response, response_content, form_data, aiohttp_session)
                except Exception as e:
...
async def get_past_image_interactions(image=None, caption=None, context="image_related", conversation_id=None, use_cache=True):
...
applications.get_swagger_ui_html = swagger_ui