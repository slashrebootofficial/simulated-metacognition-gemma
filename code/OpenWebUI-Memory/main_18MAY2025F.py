                ...
                try:
                    ...
                    async def store_interaction(user_input, response_content, base64_string):
                        ...
                        if not base64_string:
                            ...
                            # For image-related inputs, use a placeholder embedding or text description
                            document = "[IMAGE_RELATED] " + filtered_input
                            log.info("Storing image-related interaction with text description")

                        client = await chromadb.AsyncHttpClient(host=chroma_host, port=chroma_port)
                        collection = await client.get_or_create_collection(name="chat_memories")
    
                        # Always generate embeddings for the document
                        embeddings = embed_text([document])
                        if not embeddings or not embeddings[0]:
                            log.error("Failed to generate embeddings, using zero vector as fallback")
                            embeddings = [[0.0] * 768]  # all-mpnet-base-v2 produces 768-dim embeddings
                        ...
                        try:
                            ...
                            log.error(f"Failed to store interaction: {str(e)}, Type: {type(e)}")
                            raise
...
async def chat_completed(request: Request, form_data: dict, user=Depends(get_verified_user)):
    try:
        ...
        if user_input and response_content:
            ...
            if base64_string:
                metadata["raw_image_base64"] = base64_string  # Corrected from base64_embedding
