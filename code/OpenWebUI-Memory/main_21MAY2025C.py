...
#5 minute cache added for expiration to avoid stale results
        ...
        async def get_past_interactions(user_input, context="general", conversation_id=None, use_cache=True):
            ...
            try:
                ...
                text_collection = await client.get_or_create_collection(name="text_memories")
...
async def dream_during_idle():
    try:
        # Fetch admin user for context (or use a system user)
        admin_user = Users.get_user_by_role("admin")
        if not admin_user:
            log.error("No admin user found for dreaming")
            return

        # Mock request object for chat_completion_handler
        class MockRequest:
            def __init__(self):
                self.app = app
        ...
        # Randomly choose between text or image memory
