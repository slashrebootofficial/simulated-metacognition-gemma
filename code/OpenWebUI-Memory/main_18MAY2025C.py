...
# Process structured input, decoding Base64 images and system prompt
    ...
    if isinstance(input_data, list):
        ...
        for item in input_data:
            if isinstance(item, dict):
                if item.get('type') == 'system_prompt' and 'text' in item:
                    try:
                        # Decode the base64 system prompt
                        decoded_prompt = base64.b64decode(item['text']).decode('utf-8')
                        filtered_text += decoded_prompt + " "
                        log.debug(f"Decoded system prompt: {decoded_prompt[:100]}...")
                    except Exception as e:
                        log.error(f"Failed to decode base64 system prompt: {e}")
                        filtered_text += "[SYSTEM_PROMPT_DECODE_FAILED] "
    ...
    try:
        ...
        async def get_past_interactions(user_input, context="general", use_cache=True):
            ...
            try:
                ...
                # Chunk user input with overlap
                chunk_size = 384  # ~384 tokens for all-mpnet-base-v2
                overlap = 192  # 50% overlap
                input_chunks = []
                for i in range(0, len(user_input), chunk_size - overlap):
                    chunk = user_input[i:i + chunk_size]
                    input_chunks.append(chunk)
                    if i + chunk_size >= len(user_input):
                        break
                embeddings = embed_text(input_chunks)
                ...
                query_embedding = embeddings[0]  # Use first chunk for query
                log.info(f"Query embedding: {query_embedding[:10]}...")
                ...
                if results["documents"]:
                    ...
                    decay_constant = 30 * 24 * 60 * 60  # 30 days in seconds
                    combined_scores = []
                    for i, (distance, meta) in enumerate(zip(results["distances"][0][:5], results["metadatas"][0][:5])):
                        ...
                        # Apply exponential decay to access_count
                        ...
                        log.info(f"Memory {i}: Age {age_in_days:.1f} days, Decayed access_count {decayed_access_count:.2f}")
                    ...
                    for i, _ in combined_scores:
                        ...
                        for j in range(0, len(doc), chunk_size - overlap):
                            ...
                            if j + chunk_size >= len(doc):
                                break
                        response_chunks = []
                        for k in range(0, len(meta['response']), chunk_size - overlap):
                            chunk = meta['response'][k:k + chunk_size]
                            response_chunks.append(chunk)
                            if k + chunk_size >= len(meta['response']):
                                break
                        timestamp = float(meta.get('timestamp', 0))
                        ...
                        access_count = int(meta.get('access_count', 0))
                        for idx, (doc_chunk, resp_chunk) in enumerate(zip(doc_chunks, response_chunks)):
                            past_interactions += f"Past user [{time_str}] (Chunk {idx + 1}): {doc_chunk}\nLyra: {resp_chunk}\nRaw Image Base64: {raw_image_base64}\nAccess Count: {access_count}\n\n"
                else:
            ...
            if isinstance(response, StreamingResponse):
                ...
                try:
                    ...
                    async def store_interaction(user_input, response_content, base64_string):
                        ...
                        # Use the SigLIP-generated description (from response_content) as the document
                        document = filtered_response if base64_string else (filtered_input.strip() or "[NO_TEXT]")
                        client = await chromadb.AsyncHttpClient(host=chroma_host, port=chroma_port)
                        collection = await client.get_or_create_collection(
                            name="chat_memories",
                            metadata={"hnsw:M": 32, "hnsw:ef_construction": 400}
...
async def chat_completed(request: Request, form_data: dict, user=Depends(get_verified_user)):
    try:
        ...
        if user_input and response_content:
            ...
            filtered_response = response_content
            # Use the SigLIP-generated description if image present, otherwise use text input
            document = filtered_response if base64_string else (filtered_input.strip() or "[NO_TEXT]")
            if not image_embeddings and base64_string:
                log.warning("Invalid image input, no embeddings generated, using description")
