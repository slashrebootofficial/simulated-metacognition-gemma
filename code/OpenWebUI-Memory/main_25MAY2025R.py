        ...
        async def get_past_interactions(user_input, context="general", use_cache=True):
            ...
            try:
                ...
                collection = await client.get_or_create_collection(
                    name="chat_memories",
                    metadata={"hnsw:M": 32, "hnsw:ef_construction": 400, "embedding_dimension": 768}
                )
                ...
                # Filter by context and keywords (if any)
                where_clause = {"context": context}
                if query_keywords:
                    where_clause["keywords"] = {"$in": query_keywords}
            ...
            if isinstance(response, StreamingResponse):
                ...
                try:
                    ...
                    async def store_interaction(chunks, response_content):
                        ...
                        response_chunks = chunk_text(response_content)  # Chunk response separately
...
async def chat_completed(request: Request, form_data: dict, user=Depends(get_verified_user)):
    try:
        ...
        if user_input and response_content:
            chunks = process_structured_input(user_input)
            response_chunks = chunk_text(response_content)  # Chunk response
            embeddings = embed_text(chunks)
            response_embeddings = embed_text(response_chunks)
