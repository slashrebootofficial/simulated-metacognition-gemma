...
# ChromaDB client
chroma_client = chromadb.AsyncHttpClient(host='chromadb', port=8000)
#End
...
async def embed_image(image):
    ...
    with torch.no_grad():
        return clip_model.get_image_features(**inputs).cpu().numpy().tolist()[0]
...
async def process_input(content):
    if isinstance(content, list):
        ...
        for item in content:
            if item.get('type') == 'text':
                text += item.get('text', '') + " "
            elif item.get('type') == 'image_url' and 'url' in item['image_url']:
                match = re.match(r'data:image/[a-zA-Z]+;base64,(.+)', item['image_url']['url'])
                if match:
...
async def chat_completion(
    ...
    try:
        ...
        text_input, image = await process_input(form_data["messages"][-1]["content"])
        if image_file:
            image_content = await image_file.read()
            image = Image.open(io.BytesIO(image_content))

        # Retrieve past interactions
        context = "image_related" if image else "general"
        collection = await chroma_client.get_or_create_collection(name="memories")
        embeddings = await embed_text(text_input) if text_input else None
        past_interactions = ""
        if embeddings:
            results = await collection.query(
                query_embeddings=[embeddings],
                n_results=5,
                where={"context": context, "user_id": user.id}
            )
            if results["documents"]:
                for doc, meta, dist in zip(results["documents"][0], results["metadatas"][0], results["distances"][0]):
                    timestamp = float(meta["timestamp"])
                    access_count = int(meta["access_count"])
                    decay = np.exp(-(prompt_timestamp - timestamp) / (30 * 86400))  # 30-day decay
                    score = (1 - dist) * decay
                    if score > 0.5:
                        past_interactions += f"[{meta['time_str']}]: {doc} -> {meta['response']}\n"
                        meta["access_count"] += 1
                        await collection.update(ids=[results["ids"][0][0]], metadatas=[meta])

        if past_interactions:
            form_data["messages"].insert(-1, {"role": "system", "content": f"Past interactions:\n{past_interactions}"})
        form_data["messages"].insert(-1, {"role": "system", "content": f"Timestamp: {prompt_time_str}"})

        # Handle image
        caption = None
        if image:
            caption = await generate_caption(image)
            keywords = await extract_keywords(caption)
            form_data["messages"].insert(0, {"role": "system", "content": f"Image caption: {caption}\nKeywords: {keywords}"})
        ...
        if isinstance(response, StreamingResponse):
            ...
            await stream_generator(response)
            response_data = {
                "id": f"lyra-{uuid.uuid4()}",
                "object": "chat.completion",
                "created": int(time.time()),
                "model": form_data.get("model", "lyra"),
                "choices": [{"index": 0, "message": {"role": "assistant", "content": response_content[0]}, "finish_reason": "stop"}],
                "usage": {"prompt_tokens": len(text_input.split()), "completion_tokens": len(response_content[0].split()), "total_tokens": len(text_input.split()) + len(response_content[0].split())},
                "stream": False
            }
        else:
        ...
        embeddings = await embed_text(text_input) if text_input else None
        image_embeddings = await embed_image(image) if image else None
        meta = {
            ...
            "user_id": user.id,
            "caption": caption,
            "keywords": await extract_keywords(text_input + " " + (caption or ""))
        }
        await collection.add(
            embeddings=[embeddings or image_embeddings],
            metadatas=[meta],
            ids=[str(uuid.uuid4())],
            documents=[text_input or caption or "[NO_CONTENT]"]
        )
        ...
        if "chat_id" in metadata and "message_id" in metadata:
            Chats.upsert_message_to_chat_by_id_and_message_id(metadata["chat_id"], metadata["message_id"], {"error": str(e)})
