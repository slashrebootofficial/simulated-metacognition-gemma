...
def chunk_text(text, chunk_size=384, chunk_overlap=64):
    ...
    # Simplified tokenization without SpaCy
    tokens = text.split()
    chunks = []
    start = 0
...
# Process structured input, simplified to "memories" context
def process_structured_input(input_data):
    ...
    if isinstance(input_data, str):
        ...
        return chunks
    return chunk_text(str(input_data))
# End helper function
...
async def chat_completion(
    ...
    try:
        ...
        async def get_past_interactions(user_input, use_cache=True, start_timestamp=0, end_timestamp=None):
            log.info(f"Retrieving past memories for input: {user_input[:100]}...")
            if end_timestamp is None:
                end_timestamp = time.time()
            
            if use_cache and hasattr(request.state, 'memory_cache') and 'memories' in request.state.memory_cache:
                cached_time = request.state.memory_cache.get("memories_timestamp", 0)
                if time.time() - cached_time < 300:
                    log.info("Using cached memories")
                    return request.state.memory_cache['memories']
            ...
            try:
                ...
                chunks = process_structured_input(user_input)
                ...
                if embeddings is None or not embeddings:
                    ...
                    return {"role": "system", "content": "No relevant past memories found."}
                ...
                past_interactions = {"role": "system", "content": ""}
                if results["documents"] and results["documents"][0]:
                    log.info(f"Retrieved {len(results['documents'][0])} past memories with distances: {results['distances'][0]}")
                    filtered_results = [(doc, meta, dist) for doc, meta, dist in zip(results["documents"][0], results["metadatas"][0], results["distances"][0])]
                
                    if not filtered_results:
                        log.info("No memories found after context filter")
                        past_interactions["content"] = "No relevant memories found."
                    else:
                        ...
                        for i, _ in combined_scores:
                            ...
                            time_str = meta.get('time_str', 'Unknown time')
                            past_interactions["content"] += f"Memories from my Semantic Search Embeddings at {time_str} - User: {doc}\n"
                            if meta.get('response'):
                                past_interactions["content"] += f"Lyra: {meta['response']}\n\n"
                else:
                    log.info("No past memories found")
                    past_interactions["content"] = "No relevant memories found."
                
                if not hasattr(request.state, 'memory_cache'):
                    request.state.memory_cache = {}
                request.state.memory_cache['memories'] = past_interactions
                request.state.memory_cache['memories_timestamp'] = time.time()
                if len(request.state.memory_cache) > 10:
                    request.state.memory_cache.popitem(last=False)
                return past_interactions
            except Exception as e:
                log.error(f"ChromaDB retrieval failed: {e}")
                return {"role": "system", "content": "ChromaDB unavailable, relying on current context."}
        ...
        chunks = process_structured_input(form_data["messages"][-1]["content"])

        # Add timestamp as system message
        time_message = {"role": "system", "content": f"The timestamp for the current inference is {prompt_time_str}."}
        form_data["messages"].append(time_message)
        
        # Retrieve past interactions as system message
        past_interactions = await get_past_interactions(chunks[0] if chunks else "[NO_TEXT]")
        form_data["messages"].append(past_interactions)
        ...
        try:
            ...
            try:
                log.info("Storing new memories in ChromaDB at %s:%s", chroma_host, chroma_port)
                async def store_interaction(chunks, response_text, metadata):
                    client = await chromadb.AsyncHttpClient(host=chroma_host, port=chroma_port)
                    collection = await client.get_or_create_collection(
                        name="chat_memories",
                        metadata={"hnsw:M": 32, "hnsw:construction_ef": 400, "embedding_dimension": 768, "hnsw:space": "cosine"}
                    ...
                    for i, (chunk, embedding) in enumerate(zip(chunks, embeddings)):
                        ...
                        log.info(f"store_interaction user_id: {user.id}")
                        try:
                            ...
                            log.info(f"Stored input chunk {i} with timestamp: {prompt_timestamp} ({prompt_time_str})")
                        except Exception as e:
                    ...
                    for i, (chunk, embedding) in enumerate(zip(response_chunks, response_embeddings)):
                        ...
                        try:
                            ...
                            log.info(f"Stored response chunk {i} with timestamp: {prompt_timestamp} ({prompt_time_str})")
                        except Exception as e:
                            log.error(f"Failed to store response chunk {i}: {e}")

                await store_interaction(chunks, response_text, metadata)
            except Exception as e:
    ...
    try:
        ...
        if user_input and response_content:
            ...
            for i, (chunk, embedding) in enumerate(zip(chunks, embeddings)):
                ...
                log.info(f"chat_completion metadata user_id: {user.id}")
                try:
                    await collection.add(
                        embeddings=[embedding],
                        metadatas=[metadata],
            ...
            for i, (chunk, embedding) in enumerate(zip(response_chunks, response_embeddings)):
                ...
                log.info(f"response chunk user_id: {user.id}")
                try:
                    await collection.add(
                        embeddings=[embedding],
                        metadatas=[metadata],
