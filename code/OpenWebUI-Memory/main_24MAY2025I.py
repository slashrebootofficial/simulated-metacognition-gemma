...
app.state.config = AppConfig(
    redis_url=os.getenv("REDIS_URL", f"redis://{os.getenv('REDIS_HOST', 'redis')}:{os.getenv('REDIS_PORT', '6379')}/{os.getenv('REDIS_DB', '0')}"),
)
...
def embed_image(image):
    try:
        inputs = clip_processor(images=image, return_tensors="pt").to('cuda' if torch.cuda.is_available() else 'cpu')
        with torch.no_grad():
            embeddings = clip_model.get_image_features(**inputs).cpu().numpy().tolist()[0]
        return embeddings
    except Exception as e:
...
def generate_caption(image):
    try:
        ...
        log.debug(f"Generated caption: {caption}")  # Optional
        return caption
    except Exception as e:
...
def split_text_by_tokens(text: str, chunk_size: int = 384, chunk_overlap: int = 64) -> List[str]:
    ...
    try:
        ...
        if image_file:
            try:
                ...
                image = Image.open(io.BytesIO(image_content))  # Reopen after verify
            except Exception as e:
            ...
            form_data["messages"].insert(0, system_message)
        ...
        if isinstance(response, StreamingResponse):
            ...
            return {
                ...
                "usage": {
                    "prompt_tokens": count_tokens(form_data["messages"][-1]["content"]),
                    "completion_tokens": count_tokens(response_content[0]),
                    "total_tokens": count_tokens(form_data["messages"][-1]["content"]) + count_tokens(response_content[0])
                },
                "stream": False
            }
        else:
            ...
            try:
                ...
                if image_file:
                    ...
                    if image_embedding is None or len(image_embedding) != 512:  # CLIP dimension
                ...
                for chunk, embedding in zip(chunks, embeddings or [None]):
                    ...
                    if embedding and len(embedding) != 768:  # all-mpnet-base-v2 dimension
