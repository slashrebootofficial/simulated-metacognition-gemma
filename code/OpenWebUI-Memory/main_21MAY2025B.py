...
async def dream_during_idle():
    try:
        ...
        image_collection = await client.get_or_create_collection(name="image_memories")
        image_results = await image_collection.get(limit=1)
        if image_results['metadatas']:
            image_meta = image_results['metadatas'][0]
            prompt = f"Describe this image based on its tags: {image_meta['tags']}"
            response = await chat_completion_handler(
                request,
                {"messages": [{"role": "system", "content": prompt}], "model": "lyra_4:latest"},
                user
            )
            log.info(f"Lyra's idle thought: {response['choices'][0]['message']['content']}")
    except Exception as e:
        log.error(f"Dreaming error: {e}")

# Schedule dreaming task
