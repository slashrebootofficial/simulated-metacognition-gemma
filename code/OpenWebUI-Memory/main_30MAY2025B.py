...
nlp.disable_pipes("ner", "lemmatizer")  # Optimize for tokenization only
...
def process_structured_input(input_data):
    if isinstance(input_data, list):
        ...
        if not filtered_text:
            return ["[NO_TEXT]"]
        return chunk_text(filtered_text)
    if isinstance(input_data, str):
        return chunk_text(input_data)
...
async def chat_completion(
    ...
    try:
        ...
        def embed_text(texts):
            try:
                return embedding_model.encode(texts, batch_size=16, convert_to_tensor=False).tolist()
            except Exception as e:
        ...
        async def get_past_interactions(user_input, context="general", use_cache=True):
            ...
            try:
                ...
                collection = await client.get_or_create_collection(
                    name="chat_memories",
                    metadata={"hnsw:M": 32, "hnsw:construction_ef": 400, "embedding_dimension": 768}
                )
                # Log total documents to track database growth
                collection_count = await collection.count()
                log.info(f"Total documents in collection: {collection_count}")
                
                # Use first chunk and its keywords for querying
                ...
                # Query with keywords if available, otherwise omit where clause
                query_params = {
...
#                if query_keywords:
#                    query_params["where"] = {"keywords": {"$in": [json.dumps([kw]) for kw in query_keywords]}}
                results = await collection.query(**query_params)
                # Filter by context in memory
                past_interactions = ""
        ...
        context = "general"
        # Use first chunk for context retrieval
        system_content = f"Current time anchor: {prompt_time_str}"
        past_interactions = await get_past_interactions(chunks[0] if chunks else "[NO_TEXT]", context=context)
        log.debug(f"ChromaDB past interactions: {past_interactions[:4096]}...")
        if past_interactions != "No relevant past interactions found.":
        ...
        try:
            ...
            if isinstance(response, StreamingResponse):
                ...
                try:
                    ...
                    async def store_interaction(chunks, response_content):
                        client = await chromadb.AsyncHttpClient(host=chroma_host, port=chroma_port)
                        collection = await client.get_or_create_collection(
                            name="chat_memories",
                            metadata={"hnsw:M": 32, "hnsw:construction_ef": 400, "embedding_dimension": 768}
                        ...
                        if embeddings is None or not embeddings:
                            log.error("Failed to generate embeddings for input chunks")
                            embeddings = []
                        if response_embeddings is None or not response_embeddings:
                            log.error("Failed to generate embeddings for response chunks")
                            response_embeddings = []
                    ...
                    await store_interaction(chunks, filtered_response_content)
        ...
        if user_input and response_content:
            ...
            for i, (chunk, embedding) in enumerate(zip(response_chunks, response_embeddings)):
                ...
                metadata = {
                    ...
                    "time_str": prompt_time_str,
                    "context": context,
                    "event_type": "image_response" if "image" in str(user_input) else "text_response",
                    "access_count": 0,
                    "user_id": user.id,
                    "chunk_index": i,
                    "keywords": json.dumps(keywords)  # Convert to JSON string
